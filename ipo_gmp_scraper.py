...
def filter_open_mainboard_ipos(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if open_ipos:
        ...
        logger.info(f"Found {len(filtered_df)} currently open Mainboard IPOs with >=5% gains")
    else:
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            if bot_token and chat_id:
                ...
                logger.warning("⚠️ Telegram credentials not found. Set BOT_TOKEN and CHAT_ID environment variables.")
